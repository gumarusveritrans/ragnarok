...
class ConnectUpdateTopup extends Command {
	...
	public function fire()
	{
		...
		foreach ($pending_topups as $pending_topup) {
			$response = PaymentAPI::update_status('TUID114'.$pending_topup->id);
			if($response->transaction_status != "pending"){
				if($response->transaction_status == 'settlement'){
					...
					Mail::send('emails.topup', array('topup_amount' => $pending_topup->amount), function($message) use($pending_topup)
					{
